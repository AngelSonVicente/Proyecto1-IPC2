...
<%
    ...
    String CodigoPedido=request.getParameter("pedidos");
String IDEnvio=request.getParameter("CE");
...
<form method="post" action="ActualizarPedido.jsp">

    <input type="hidden" name="costoTotal" id="costo-total-input">
    Codigo del pedido: <input type ="text" name="CodigoPedido" value="<%=CodigoPedido%>" readonly/>
    ...
    <select id="select-carrito" name="carrito" multiple>


        <%
            ArrayList<ListaPedido> listaEnvio = datos.DetallePedido(CodigoPedido);%>
        <% for (ListaPedido producto : listaEnvio) { %>
        <option value="<%= producto.getCodigoProducto() %>,<%=producto.getCantidad()%>" data-cantidad="<%= producto.getCantidad() %>" >
            -Codigo Producto: <%= producto.getCodigoProducto() %> - Nombre:  <%= producto.getNombreProducto() %> -Cantidad: <%=producto.getCantidad()%> - Costo <%=producto.getCostoU()%>
        </option>
    ...
    <input type="hidden" name="CE" value="<%=IDEnvio%>">
